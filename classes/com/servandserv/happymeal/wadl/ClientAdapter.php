...
class ClientAdapter {
        ...
        foreach ($conf as $name => $method) {
            ...
            if (strtoupper($rm) === "POST") {
                ...
                if (array_key_exists("CONTENT_TYPE", $_SERVER) &&
                    ...
                    if ($xr->XML($GLOBALS["HTTP_RAW_POST_DATA"])) {
                        ...
                        $cli->throwException("Error on XML request decode", 400);
                    }
                    ...
                    if ($json = json_decode($GLOBALS["HTTP_RAW_POST_DATA"], TRUE)) {
                        ...
                        $cli->throwException("Error on JSON request decode", 400);
                    }
                } else {
                    $cli->reqArray = $_POST;
                }
            ...
            if (isset($method["request"]) && isset($method["request"]["representation"])) {
                foreach ($method["request"]["representation"] as $mediaType) {
                    if ($cli->reqXML !== NULL && strpos($mediaType[0], "/xml")) {
                        $media = TRUE;
                    }
                    if ($cli->reqJSON !== NULL && strpos($mediaType[0], "/json")) {
                        $media = TRUE;
                    }
                }
            } else if ($cli->reqArray !== NULL) {
                $media = TRUE;
            }

            if (!$media) {
                $cli->throwException("Unsupported Media Type", 415);
            }
    ...
    public function response(AnyType $response = NULL, $code = 200, $pi = NULL) {
        if ($this->router && $this->router->redirect($this->request, $response)) {
            return;
        }
        if ($response === NULL) {
    ...
    private function accept() {
        ...
        $parts = preg_split('/\s*(?:,*("[^"]+"),*|,*(\'[^\']+\'),*|,+)\s*/', $_SERVER["HTTP_ACCEPT"], 0,
