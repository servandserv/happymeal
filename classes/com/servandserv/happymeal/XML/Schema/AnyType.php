...
class AnyType implements MarkupArrayAdaptor, JSONAdaptor, XMLAdaptor 
{
    ...
    {
        foreach( $vars as $k=>$v ) {
            if(preg_match(\com\servandserv\happymeal\MarkupArrayAdaptor::NSS,$k,$m)) {
                $nss[$k] = $v;
            }
        }
        return $nss;
    ...
    protected function treeFromPath( array $path, array &$tree, $v, array $nss, $prefix = NULL ) 
    {
        ...
        $name = preg_split("/\\" . \com\servandserv\happymeal\MarkupArrayAdaptor::NS_DELIMITER . "/", $node, -1, PREG_SPLIT_NO_EMPTY);
        if( count( $name ) > 1 ) {
            $prefix = $name[0];
            array_shift( $name );
        }
        $pos = preg_split("/\\" . \com\servandserv\happymeal\MarkupArrayAdaptor::POS_DELIMITER . "/", $name[0], -1 );
        $localName = $pos[0];
        $nodeType = "elements";
        $position = 0;
        if( count( $pos ) > 1 ) {
