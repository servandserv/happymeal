...
class AnyComplexType extends AnyType {
    ...
    const ANY_VALUE = 0;
    const NOT_NULL = 1;
    const VALIDATOR_CLASS = 'com\servandserv\happymeal\xml\schema\AnyComplexTypeValidator';
    ...
    public function validateType(ErrorsHandler $handler) {
        $validator = Bindings::create(self::VALIDATOR_CLASS, [$this, $handler]);
        $validator->validate();
    }
            ...
            foreach ($this->__props as $k => $prop) {
                ...
                if (( $prop["xmlns"] === $nodeData["ns"] || $nodeData["ns"] === NULL ) &&
                    ...
                    if ($prop["class"] && $nodeData["complex"] === TRUE) {
                        ...
                        foreach ($nodeData["elements"] as $el) {
                            call_user_func(
                                array($this, $prop["setter"]), trim($el)
                            );
                        }
                    }
                }
