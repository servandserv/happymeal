    ...
    public function validate() {
        foreach ($this->__props as $k => $prop) {
            if (method_exists($this->tdo, $prop["getter"])) {
                ...
                if ($val === NULL && $prop["minOccurs"] == 0) {
                    continue;
                }
                $vals = is_array($val) ? $val : [$val];
                $this->validatePropVals($prop, $vals);
            }
        }
    }

    private function validatePropVals($prop, $vals) {
        foreach ($vals as $val) {
            if (is_object($val) && method_exists($val, "validateType")) {
                $val->validateType($this->validationHandler);
            } else {
                $this->validatePropVal($prop, $val);
            }
        }
    }

    private function validatePropVal($prop, $val) {
        $prototype = Bindings::create($prop["prototype"], [$val]);
        $params = [$prototype, $this->validationHandler];
        $v = Bindings::create($prop["validator"], $params);
        $v->validate();
    }
